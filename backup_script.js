...
// --- NOVOS ELEMENTOS (CORREÇÃO E MODAL) ---
...
function deleteCurrentCard() {
    ...
    // Remove do banco de dados principal (allQuestions)
    ...
    if (indexInAll > -1) {
        allQuestions.splice(indexInAll, 1);
    }

    // Remove do jogo atual (questionsPool)
    if (currentQuestionIndexInPool > -1) {
...
function saveEditedCard() {
    // 1. Atualizar os dados do objeto (Referência em memória)
    ...
    // 2. Atualizar a UI imediatamente (para refletir na tela de erro)
    // Se a questão foi atualizada, atualizamos o texto da pergunta
    // E, se estivermos na tela de feedback (erro), atualizamos a resposta exibida
    updateFeedbackText();
    
    // 3. Salvar estado e fechar
    saveGameState();
    closeEditModal();
}

function updateFeedbackText() {
    // Esta função redesenha o texto da questão com base no estado atual (Corrigindo ou Pergunta normal)
...
function resetUI() {
    ...
    // Esconde a barra de opções de correção ao carregar nova questão
...
function showFeedback(isCorrect, element) {
    ...
    if (element) { // Múltipla escolha
        ...
        if (!isCorrect) {
            ...
            updateFeedbackText(); // Chama a função centralizada de exibição
            
            // Lógica de Inputs para Correção
            if (currentQuestion.type === 'open_double') {
            ...
            // MOSTRA AS OPÇÕES DE CORREÇÃO (EDITAR/EXCLUIR)
